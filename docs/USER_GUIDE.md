...
*   **Auto-exposure**: enabled only where the host drives the metering. It is greyed out on an LS-50 / Coolscan V over nkscan, which is not the same as having none — that scanner meters in firmware on the first frame of every pass and holds the result down the strip, with nothing to switch on or off.
*   **Frame range**: appears for any adapter that can position film. On nkscan the upper bound is the frame count the loaded carrier reports — a six-frame strip in an SA-21 offers 1–6. A carrier that reports only one frame (which is also what a strip reads once it has been scanned through) falls back to the six-frame bound rather than collapsing the range.
*   **Lock white balance** / **Lock exposure across batch**: nkscan-specific, shown once a frame range exists, and independent of each other. **Lock white balance** holds white balance during every frame's own metering, so the film's cast isn't scanned out; it appears only where the host does the metering, so a scanner that meters in firmware (the LS-50 / Coolscan V) does not offer it. **Lock exposure across batch** meters only the first frame, then holds that exact exposure for every remaining frame. Combine both for roll analysis (consistent color and brightness across the whole strip); either can be used alone. Both off by default.

A batch scan holds the scanner open for the whole frame range rather than reopening per frame, regardless of backend. On nkscan the whole strip is positioned in one go before the first frame is read, so the batch declares every frame position up to the last one ticked — including the offset and drift for each — rather than settling them frame by frame.
